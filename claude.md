...
- **Vite 7.0.1** - 构建工具
- **Tailwind CSS 4.1** + **shadcn/ui** - UI框架和组件库
- **Lucide React** - 图标库
...
**shadcn/ui 优先原则**：实现界面时优先使用 shadcn/ui 组件库，没有合适的组件时先查阅文档安装，再考虑自定义实现。

- **导入方式**：`import { Button } from '@/components/ui/button'`
- **安装命令**：`npx shadcn@latest add [组件名] --yes`
- **样式定制**：使用 `cn()` 函数组合 Tailwind 类名
- **设计规范**：自定义组件遵循 shadcn/ui 的代码风格
...
**不要在代码、文档、git提交信息中特意写出AI助手（比如Claude）的相关信息**

## 开发笔记管理

开发过程中的经验教训和技术解决方案记录在 [`dev-notes/`](dev-notes/) 文件夹中，包括：

- **实现详解**：重要功能的完整实现过程（如 [WebContainer 实现](dev-notes/webcontainer-implementation.md)、[shadcn/ui 集成](dev-notes/shadcn-integration.md)）
- **踩坑记录**：常见错误和解决方案
- **API 参考**：库和框架的使用经验
- **架构决策**：技术选型和设计思路
